(function() {
  ...
  const extractAverageColor = (url) => new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (!count) return resolve({ r: 88, g: 196, b: 181 });
  ...
  // ---------------------------------------------
  // AUTO-CHANGING FAVICON (folder-based system)
  // ---------------------------------------------
  function applyEventFavicon() {
    ...
    // Default folder
    let path = "assets/normal/";

    // Pride Month (June)
    if (m === 6) path = "assets/pride/";

    // Halloween (October)
    if (m === 10) path = "assets/halloween/";

    // Christmas (Dec 1–26)
    if (m === 12 && d <= 26) path = "assets/christmas/";

    // Apply main favicon
    link.href = path + "favicon-32x32.png";

    // Update fallback icons
    const ico = document.querySelector("link[rel='shortcut icon']");
    if (ico) ico.href = path + "favicon.ico";

    const apple = document.querySelector("link[rel='apple-touch-icon']");
    if (apple) apple.href = path + "apple-touch-icon.png";
  }
    ...
    sites.forEach((site, idx) => {
      ...
      li.innerHTML = `
        ...
        <div class="actions">
          ${
            isEmpty
              ? `<span class="badge"><i class="fa-solid fa-hourglass-half"></i> Pending</span>`
